...
public class CustomerCashPaymentQuerySoapRequestBuilderTest {
    ...
    public void testBuildRequest() {
        ...
        AccountingTransactionRequest req = fact
                .createAccountingTransactionRequest();

        HeaderType head = HeaderTypeBuilder.Builder
                .create()
                ...
                .withTransaction(ApiTransactionCodes.ACCOUNTING_CASHRECEIPT_GET)
        ...
        xb.setXactTypeId(BigInteger.valueOf(2));
        xb.setXactDate("2018-12-01");
        xb.setConfirmNo("ADB-49384343");
        xb.setTenderId(BigInteger.valueOf(100));

        XactCustomRelationalCriteriaType customCriteria = fact
                .createXactCustomRelationalCriteriaType();

        criteria.setBasicCriteria(xb);
        criteria.setCustomCriteria(customCriteria);

        TransactionCriteriaGroup criteriaGroup = fact
                .createTransactionCriteriaGroup();
        ...
        Assert.assertTrue(bodyXml.contains(ApiTransactionCodes.ACCOUNTING_CASHRECEIPT_GET));
        SoapMessageHelper util = new SoapMessageHelper();
        String soapXml = util.createRequest(bodyXml);
        Assert.assertNotNull(soapXml);
        Assert.assertTrue(soapXml.contains(ApiTransactionCodes.ACCOUNTING_CASHRECEIPT_GET));
