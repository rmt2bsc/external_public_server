...
public class SalesOrderDeleteSoapRequestBuilderTest {
    ...
    public void testBuildRequest() {
        ...
        HeaderType head = HeaderTypeBuilder.Builder.create().withApplication("accounting").withModule("transaction")
                ...
                .withTransaction(ApiTransactionCodes.ACCOUNTING_SALESORDER_DELETE).withRouting(ApiHeaderNames.DUMMY_HEADER_VALUE)
        ...
        Assert.assertTrue(bodyXml.contains(ApiTransactionCodes.ACCOUNTING_SALESORDER_DELETE));
        SoapMessageHelper util = new SoapMessageHelper();
        String soapXml = util.createRequest(bodyXml);
        Assert.assertNotNull(soapXml);
        Assert.assertTrue(soapXml.contains(ApiTransactionCodes.ACCOUNTING_SALESORDER_DELETE));
