...
public class UserGroupDeleteSoapRequestBuilderTest {
    ...
    public void testBuildRequest() {
        ...
        UserGroupType rt = UserGroupTypeBuilder.Builder.create()
                .withGroupId(777)
                .withDescription("Group Description")
                .build();

        apgt.getUserGroupInfo().add(rt);
        req.setProfile(apgt);
        req.setHeader(head);
        ...
        Assert.assertTrue(bodyXml.contains(ApiTransactionCodes.AUTH_USER_GROUP_DELETE));
        SoapMessageHelper util = new SoapMessageHelper();
        String soapXml = util.createRequest(bodyXml);
        Assert.assertNotNull(soapXml);
        Assert.assertTrue(soapXml.contains(ApiTransactionCodes.AUTH_USER_GROUP_DELETE));
